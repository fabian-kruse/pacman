...
(function () {
	...
	google.util.escape = function (a) {
		for (var b = 0; b < y.length; b += 2)
			a = a.replace(new RegExp(y[b], 'g'), y[b + 1]);
		return a;
	};
	google.util.unescape = function (a) {
		for (var b = 0; b < y.length; b += 2)
			a = a.replace(new RegExp(y[b + 1], 'g'), y[b]);
		return a;
	};
...
(function () {
	...
	function m(b, c, a) {
		for (var e = document.getElementsByTagName('a'), o = 0, h; (h = e[o++]); )
			b(h) && google.bind(h, 'click', l(c, a));
	}
	function n() {
		if (!g.persisted) {
			g.persisted = true;
	...
	function Qa() {
		...
		if (n) {
			...
			a(
				'.gac_od',
				c +
					'border-top:0;border-left:0;border-right:1px solid #e7e7e7;border-bottom:1px solid #e7e7e7;padding:0!important;position:absolute'
			);
			a(
				'.gac_id',
				c +
	...
	function Za(a) {
		if (!(p != q || !r || !r.length))
			if (Wa()) {
				if (s) s.className = 'gac_a';
				for (
					var b = r.length, c = ((u + 1 + (a ? 1 : b)) % (b + 1)) - 1;
					c != -1 && W(c).t;
		...
		Hb = {
			...
			e: function (a, b, c, d) {
				...
				a.innerHTML = [
					...
					')</a><br><b>',
					g,
					'</b> <span class=gac_y><span class=gac_',
					k >= 0 ? 'h' : 'r',
					'>',
					k,
					' (',
		...
		Gb = {
			...
			e: function (a, b, c, d) {
				...
				Nb(
					...
					'www.google.com',
					[c, ' - <span class=gac_i>', d, '</span>'].join('')
				);
			},
		},
		Ab = {
...
(function () {
	...
	function q(a) {
		if (!a.A) {
			...
			a.a.F = function (b, c) {
				...
				return (
					b > e - 5 &&
					b < e + google.getWidth(a.a) + 5 &&
					c > f - 5 &&
					c < f + google.getHeight(a.a) + 5
				);
			};
	...
	function v(a) {
		...
		if (!a.v) {
			...
			a.H =
				b &&
				google.getPageOffsetStart(a.d) >
					google.getPageOffsetStart(b) + google.getWidth(b);
...
(function () {
	...
	function r(a, b) {
		for (var c = b ? 1 : 0, d = b ? 0 : 1; c < q.length; c += 2, d += 2)
			a = a.replace(new RegExp(q[c], 'g'), q[d]);
		return a;
	}
	function s(a) {
	...
	function y(a) {
		...
		if (a.style.backgroundPosition) {
			...
			a.style.backgroundPosition = /nav_logo4.png/.test(
				u('img', s('logo'), 1)[0].src
			)
				? '-126px -78px'
				: '-153px -84px';
			return i;
			...
			function (d) {
				if (d.channelHeader.token) {
					...
					if (d.swToken) {
						k.qt = d.swToken;
						k.at = {};
						for (var e = 0, f; (f = d.queryTokens[e++]); )
							k.at[f.query] = f.token;
					}
	...
	function N(a) {
		...
		if (!a.f) {
			a.f = n(
				'div.wcd',
				p(
					'<span style=color:#666>Comment by {name}, <span class=wcu></span></span>&nbsp;- "<span class=wct></span>" <a href=# class="wcl fl">Edit</a>',
					{
						name: k.nn,
					}
				)
	...
	function O(a) {
		...
		if (b != a.e) {
			u('.wct', a.a).innerHTML = r(b).replace(
				/(\S{25}[^&\s]{5})(\S{10})/g,
				'$1<wbr>$2'
			);
	...
	function ha(a) {
		...
		a.r = n(
			'div.wcd',
			'<span style=color:#666>Comment deleted </span> - <a href=# class=fl>Undo</a>'
		);
		a.m.value = '';
		O(a);
	...
	var ma = function (a) {
			function b() {
				if (!R && !I) {
					var c = n(
						'li.g',
						'Results you remove can be viewed at the bottom of the page.'
					);
					v(c, a.a);
					window.setTimeout(function () {
				...
				V(a);
				if (!a.k)
					for (var d = a.g + 1, e; (e = E[d++]) && e.k; )
						if (e.b == 0) e.a.style['marginLeft'] = '';
			}
			if (I) b();
			else {
				la(a, b);
	...
	var ra = function (a) {
			var b = a.j.className == 'ws',
				c = b ? 'wsa' : 'ws';
			a.j.className = c;
			for (var d = 0, e; (e = E[d]); ++d)
				if (d != a.g && e.c == a.c && e.j) e.j.className = c;
			if (!I) {
				b
					? F(a, 0)
					: C('delete', {
		...
		sa = function () {
			...
			u('a', a).innerHTML = b
				? 'Show more starred results'
				: 'Hide more starred results';
		...
		ua = function (a, b) {
			...
			va(a, d, function () {
				...
				if (!a.k)
					for (f = a.g + 1; (h = E[f++]) && h.k; )
						if (h.b == 0) h.a.style['marginLeft'] = h.k;
			});
			if (!I && !b) {
				F(a, 1);
				L(0, a.g, a.b);
		...
		na = function (a, b) {
			x(
				0,
				'/search?q=info:' + l(a + ' ' + k.q) + '&swm=5&hl=' + google.kHL,
				function (c) {
					if ((c = c && c.match(pa)) && /class=ws/.test(c[1]))
						b(n('div', c[1]).firstChild);
					else {
						...
						b(
							n(
								'li.g',
								p(
									'<h3 class=r><a class=l href="{url}">{url}</a></h3><button class=ws></button><div class=s><cite>{site}</cite></div><div class=wce></div>',
									{
										url: r(a),
		...
		ka = function (a) {
			b = parseInt(
				window.getComputedStyle(a, g).getPropertyValue('margin-bottom'),
				10
			);
			return b;
		},
		W = function (a, b) {
			for (var c = a; (c = c.nextSibling); )
				if (c.nodeType == 1 && ((c.innerHTML && c.offsetHeight > 0) || c == b))
					return (!/\bg\b/.test(c.className) && u('.g\\b', c, 1).pop()) || c;
	...
	function za() {
		...
		Y = n('div');
		Aa();
		for (var a = (K = 0), b; (b = E[a++]); )
			K = (K << 5) ^ (K >> 27) ^ b.c.length ^ (b.b << 8);
	...
	function Aa() {
		...
		for (var b = 0, c; (c = a[b++]); )
			ja(c, parseInt(c.className.match(/\bw(\d)/)[1], 10));
	}
	...
	function Ba(a) {
		if (this.className in $) {
			this.style.backgroundPosition =
				a.type == 'mouseout'
					? ''
					: -$[this.className][0] + 'px ' + -$[this.className][1] + 'px';
		}
	}
...
(function () {
	...
	function ma(a, c, b, d) {
		...
		e = [
			[
				K,
				'marginTop',
				google.getComputedStyle(K, 'margin-top'),
				e,
	...
	function da(a, c, b, d) {
		...
		if (d) H(a, c);
			...
			qa = window.setInterval(function () {
				H(a, c);
			}, 6e4);
	}
	google.dstr.push(function () {
