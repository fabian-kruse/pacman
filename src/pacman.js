...
google.pacman ||
	(function () {
		...
		Entity.prototype.A = function () {
			...
			this.c(0);
			this.reverseDirectionsNext =
				this.freeToLeavePen =
				this.modeChangedWhileInPen =
		...
		Entity.prototype.i = function (b) {
			...
			if (h.intersection)
				switch (this.mode) {
					...
					case 8:
						if (
							(this.dir & h.allowedDir) == 0 &&
							h.allowedDir == g.oppositeDirections[this.dir]
						)
					...
					case 4:
						if (
							...
							while (
								(f & h.allowedDir) == 0 ||
								f == g.oppositeDirections[this.dir]
							);
							this.nextDir = f;
						}
						break;
		...
		Entity.prototype.p = function (b) {
			...
			g.playfield[b[0]][b[1]].type == 1 && this.mode != 8
				? this.c(2)
				: this.c(0);
		...
		Entity.prototype.B = function () {
			if (
				this.id == g.playerCount &&
				...
				switch (this.id) {
					...
					case g.playerCount + 3:
						...
						this.targetPos =
							c > 64 ? [b.tilePos[0], b.tilePos[1]] : this.scatterPos;
						break;
				}
			}
		};
		Entity.prototype.v = function () {
			this.routineMoveId++;
		...
		Entity.prototype.j = function () {
			if (this.routineMoveId == -1 || this.proceedToNextRoutineMove) this.v();
			this.m();
		};
		Entity.prototype.d = function () {
			switch (this.currentSpeed) {
				case 0:
					var b =
						this.id == g.playerCount && (this.mode == 2 || this.mode == 1)
							? g.cruiseElroySpeed
							: this.fullSpeed;
					break;
				case 1:
		...
		Entity.prototype.c = function (b) {
			this.currentSpeed = b;
			this.d();
		};
		Entity.prototype.e = function () {
			if (this.dir)
				if (this.speedIntervals[g.intervalTime]) {
					var b = l[this.dir];
		...
		Entity.prototype.s = function () {
			var b = 0,
				c = 0,
				direction = this.dir;
			if (direction == 0) direction = this.lastActiveDir;
			if (g.gameplayMode == 1 && this.id == g.playerEatingGhostId) {
				b = 3;
				c = 0;
			} else if (
				(g.gameplayMode == 9 || g.gameplayMode == 10) &&
				this.id == 0
			) {
				b = 2;
				c = 0;
				...
				if (this.id == g.playerDyingId) {
					direction = 20 - Math.floor((g.gameplayModeTime / g.timing[4]) * 21);
					if (this.id == 0) {
						b = direction - 1;
						switch (b) {
							case -1:
				...
				b = 14;
				c = 0;
				direction = Math.floor(g.globalTime * 0.2) % 4;
		...
		Entity.prototype.r = function () {
			...
			if (g.gameplayMode == 10 || g.gameplayMode == 4 || g.gameplayMode == 3) {
				...
				c = 8;
				if (
					g.frightModeTime < g.levels.frightTotalTime - g.levels.frightTime &&
		...
		Entity.prototype.b = function () {
			...
			var b = [0, 0];
			b =
				g.gameplayMode == 8 || g.gameplayMode == 14
					? [0, 3]
					: this.ghost
					? this.r()
					: this.s();
		...
		g.getDistance = function (b, c) {
			return Math.sqrt(
				(c[1] - b[1]) * (c[1] - b[1]) + (c[0] - b[0]) * (c[0] - b[0])
			);
		};
		...
		g.prepareElement = function (b, c, d) {
			...
			if (g.useCss) {
				b.style.backgroundImage = 'url(../img/pacman10-hp-sprite-2.png)';
				b.style.backgroundPosition = -c + 'px ' + -d + 'px';
				b.style.backgroundRepeat = 'no-repeat';
			} else {
				b.style.overflow = 'hidden';
				c =
					'display: block; position: relative; left: ' +
					-c +
					'px; top: ' +
					-d +
					'px';
				b.innerHTML =
					'<img style="' + c + '" src="../img/pacman10-hp-sprite.png">';
			}
		};
			...
			for (var b in n) {
				...
				if (c.w) {
					for (var f = c.y * 8, h = c.x * 8; h <= (c.x + c.w - 1) * 8; h += 8) {
						...
						g.playfield[f][h].type =
							!d || (h != c.x * 8 && h != (c.x + c.w - 1) * 8) ? d : 0;
					}
		...
		g.registerTouch = function () {
			...
			document.f &&
				document.f.q &&
				document.f.q.addEventListener('touchstart', g.handleTouchStart, a);
		};
		...
		g.restartGameplay = function (b) {
			...
			g.ghostModeTime = g.levels.ghostModeSwitchTimes[0] * default_fps;
			g.ghostExitingPenNow = e;
			g.ghostEyesCount = 0;
			g.tilesChanged = e;
			g.updateCruiseElroySpeed();
		...
		g.createKillScreenElement = function (b, c, d, f, h) {
			...
			if (h) {
				j.style.background =
					'url(../img/pacman10-hp-sprite.png) -' +
					g.killScreenTileX +
					'px -' +
					g.killScreenTileY +
					'px no-repeat';
		...
		g.newLevel = function (b) {
			g.level++;
			g.levels =
				g.level >= different_levels.length
					? different_levels[different_levels.length - 1]
					: different_levels[g.level];
			g.levels.frightTime = Math.round(g.levels.frightTime * default_fps);
			g.levels.frightTotalTime =
				g.levels.frightTime + g.timing[1] * (g.levels.frightBlinkCount * 2 - 1);
		...
		g.figureOutPenLeaving = function () {
			if (g.alternatePenLeavingScheme) {
				...
				g.actors[g.playerCount + 1].mode == 16 ||
				g.actors[g.playerCount + 1].mode == 8
			) {
				g.actors[g.playerCount + 1].dotCount++;
				if (
					g.actors[g.playerCount + 1].dotCount >= g.levels.penLeavingLimits[1]
				)
					g.actors[g.playerCount + 1].freeToLeavePen = a;
			} else if (
				g.actors[g.playerCount + 2].mode == 16 ||
				g.actors[g.playerCount + 2].mode == 8
			) {
				g.actors[g.playerCount + 2].dotCount++;
				if (
					g.actors[g.playerCount + 2].dotCount >= g.levels.penLeavingLimits[2]
				)
					g.actors[g.playerCount + 2].freeToLeavePen = a;
			} else if (
				g.actors[g.playerCount + 3].mode == 16 ||
				g.actors[g.playerCount + 3].mode == 8
			) {
				g.actors[g.playerCount + 3].dotCount++;
				if (
					g.actors[g.playerCount + 3].dotCount >= g.levels.penLeavingLimits[3]
				)
					g.actors[g.playerCount + 3].freeToLeavePen = a;
			}
		};
		g.resetForcePenLeaveTime = function () {
			g.forcePenLeaveTime = g.levels.penForceTime * default_fps;
		};
		...
		g.detectCollisions = function () {
			...
			for (var b = g.playerCount; b < g.playerCount + 4; b++)
				for (var c = 0; c < g.playerCount; c++)
					if (
						g.actors[b].tilePos[0] == g.actors[c].tilePos[0] &&
						g.actors[b].tilePos[1] == g.actors[c].tilePos[1]
					)
		...
		g.changeGameplayMode = function (b) {
			...
			switch (b) {
				...
				case 4:
					...
					g.stopAllAudio();
					g.playerCount == 2
						? g.playSound('start-music-double', 0, a)
						: g.playSound('start-music', 0, a);
					break;
				case 5:
					g.lives--;
					g.updateChromeLives();
		...
		g.updateSoundIcon = function () {
			if (g.soundEl)
				google.pacManSound
					? g.changeElementBkPos(g.soundEl, 216, 105, e)
					: g.changeElementBkPos(g.soundEl, 236, 105, e);
		};
		...
		g.cutsceneNextSequence = function () {
			...
			if (g.cutscene.sequence.length == g.cutsceneSequenceId) g.stopCutscene();
				...
				g.cutsceneTime = b.time * default_fps;
			...
			if (g.gameplayModeTime) {
				...
				switch (g.gameplayMode) {
					...
					case 3:
						...
						break;
					case 10:
						Math.floor(g.gameplayModeTime / (g.timing[11] / 8)) % 2 == 0
							? g.changeElementBkPos(g.playfieldEl, 322, 2, e)
							: g.changeElementBkPos(g.playfieldEl, 322, 138, e);
				}
					...
					switch (g.gameplayMode) {
						case 1:
							...
							for (b = g.playerCount; b < g.playerCount + 4; b++)
								if (
									g.actors[b].mode == 4 ||
									((g.actors[b].mode == 16 || g.actors[b].mode == 128) &&
										!g.actors[b].eatenInThisFrightMode)
								) {
									c = a;
									break;
								}
			...
			if (g.frightModeTime) {
				...
				if (g.ghostModeTime <= 0) {
					...
					if (g.levels.ghostModeSwitchTimes[g.ghostModeSwitchPos]) {
						g.ghostModeTime =
							g.levels.ghostModeSwitchTimes[g.ghostModeSwitchPos] * default_fps;
		...
		g.updateChromeScore = function (b) {
			...
			for (var d = 0; d < g.scoreDigits; d++) {
				var f = document.getElementById('pcm-sc-' + (b + 1) + '-' + d),
					h = c.substr(d, 1);
				h
					? g.changeElementBkPos(f, 8 + 8 * parseInt(h, 10), 144, a)
					: g.changeElementBkPos(f, 48, 0, a);
			}
		};
		...
		g.updateChromeLevel = function () {
			...
			for (var b = g.level; b >= Math.max(g.level - 4 + 1, 1); b--) {
				var c =
						b >= different_levels.length
							? different_levels[different_levels.length - 1].fruit
							: different_levels[b].fruit,
		...
		g.decreaseFps = function () {
			if (g.fpsChoice < fps.length - 1) {
				g.fpsChoice++;
				g.initializeTickTimer();
				if (g.fpsChoice == fps.length - 1) g.canDecreaseFps = e;
			}
		};
		g.addCss = function () {
			var b =
		...
		g.prepareGraphics = function () {
			g.graphicsReady = e;
			g.preloadImage('../img/pacman10-hp-sprite.png');
		};
		...
		g.flashLoaded = function () {
			if (g.flashIframeDoc) {
				...
				if (b && b.playTrack) {
					...
					(b = g.flashIframeDoc.getElementById('pacman-sound-player-2')) &&
					b.playTrack
				) {
					g.flashReady(b);
					return;
